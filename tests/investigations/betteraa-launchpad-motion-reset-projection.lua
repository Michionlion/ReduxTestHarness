Test.name("Redux Better AA launchpad reset projection-matrix state")
...
Test.capture.screenshot(
    "before-reset-projection",
    { scale = 1, hideUI = false, waitFrames = 1 })

Test.report.value(
    "override",
    better_aa.override_camera_state("resetProjection"))
Test.wait.frames(24)
Test.report.value("afterReset", better_aa.matrix_snapshot())
Test.capture.screenshot(
    "after-reset-projection",
    { scale = 1, hideUI = false, waitFrames = 1 })

Test.report.note(
    "Reset only Unity's projection-matrix override state after reproducing the raw launchpad motion field; no sanitizer or AA backend was active")
